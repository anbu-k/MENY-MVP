...
var dutch_grant_lots_info = [], farms_grants_info = [], settlements_info = [], infos_entity = [], settlements_linked_location = [], brooklyn_grants_info = [], taxlot_events_info = [], taxlot_event_entities_info = [];
var dutch_grant_lots_info_length = 0, farms_grants_info_length = 0, settlements_info_length = 0, infos_entity_length = 0, brooklyn_grants_length = 0, taxlot_events_info_length = 0, taxlot_entities_info_length = 0;
//#endregion
...
var tooltiPos = -100;
var ruler_step = (sliderEnd - sliderStart) / 10, date_ruler1 = sliderStart + ruler_step, date_ruler2 = sliderStart + ruler_step * 3, date_ruler4 = sliderStart + ruler_step * 7, date_ruler5 = sliderStart + ruler_step * 9;
//#endregion
...
$(document).ready(function () {
    ...
    if (jQuery.browser.msie)
        alert("Sorry, this application uses state of the art HTML5 techniques which are not (well) supported by Internet Explorer.\nUse Google Chrome or Mozilla Firefox to experience the full power of HTML5 and this application");
    //#endregion
    ...
    windoWidth = (_a = $(window).width()) !== null && _a !== void 0 ? _a : -1;
    if (windoWidth <= 637) {
        if (layer_view_flag) {
    ...
    // $("#date").text(
    //   (moment.unix($("#slider") as any).slider("values", 0)).format("DD MMM YYYY")
    // );
    //#endregion
    ...
    $("#grants_layer").click(function () {
        if ($(this).prop("checked")) {
            ...
            ["dutch_grants-5ehfqe", "dutch_grants-5ehfqe-highlighted"].forEach(function (id) {
                setLayoutVisibilityForBothMaps(id, "visible");
            });
        }
        else {
    ...
    $(".native_groups_layer").change(function () {
        if ($(".native_groups_layer:checked").length ==
    ...
    $("#native_groups_area").click(function () {
        if ($(this).prop("checked")) {
            ["native-groups-area", "native-groups-area-highlighted"].forEach(function (id) {
                setLayoutVisibilityForBothMaps(id, "visible");
            });
        }
        else {
    ...
    $("#circle_point").click(function () {
        if ($(this).prop("checked")) {
            ...
            [0, 1].forEach(function (index) {
                if (index === 0)
                    beforeMap.setLayoutProperty("lot_events-bf43eb", "visibility", "visible");
                else
                    afterMap.setLayoutProperty("lot_events-bf43eb", "visibility", "visible");
            });
        }
        else {
            [0, 1].forEach(function (index) {
                if (index === 0)
                    beforeMap.setLayoutProperty("lot_events-bf43eb", "visibility", "none");
                else
                    afterMap.setLayoutProperty("lot_events-bf43eb", "visibility", "none");
            });
            if (demo_layer_view_flag) {
                closeDemoInfo();
            }
