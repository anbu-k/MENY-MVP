...
var na_bearing = -51.3, na_center = [-74.01255, 40.704882], na_zoom = 16.34;
// #endregion
...
function demoFilterRangeCalc() {
    ...
    if (demo_layer_features.length > 0) {
        var sliderStartMin = null, sliderEndMax = null, curr_start_timestamp = null, curr_end_timestamp = null;
        for (var i_1 = 0; i_1 < demo_layer_features.length; i_1++) {
            if (demo_layer_features[i_1].properties.TAXLOT == demo_layer_taxlot) {
                demo_layer_feature_props = demo_layer_features[i_1].properties;
                if (typeof demo_layer_features[i_1].properties.DayStart === "undefined" ||
                    demo_layer_features[i_1].properties.DayStart === null)
                    curr_start_timestamp = null;
                else
                    curr_start_timestamp = moment(demo_layer_features[i_1].properties.DayStart, "YYYYMMDD").unix();
                if (typeof demo_layer_features[i_1].properties.DayEnd === "undefined" ||
                    demo_layer_features[i_1].properties.DayEnd === null)
                    curr_end_timestamp = null;
                else
                    curr_end_timestamp = moment(demo_layer_features[i_1].properties.DayEnd, "YYYYMMDD").unix();
        ...
        if (sliderStartMin !== null && sliderEndMax !== null) {
            ...
            if (demo_layer_view_flag)
                buildPopUpInfo(demo_layer_feature_props, "#demoLayerInfo");
        }
    }
}
// #endregion
...
function addLayers() {
    ...
    labelData.forEach(function (labelInfo) {
        var labelObject = createLabel(labelInfo.title, labelInfo.coordinates, labelInfo.minZoom);
